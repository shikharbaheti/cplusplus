...
Image::Image(const Image& oldOne){
  ...
  allocateImage(oldOne.width, oldOne.height);

  for (unsigned int i = 0; i < oldOne.height; i++){
    for (unsigned int j = 0; j < oldOne.width; j++){
      image[j][i].setRed(oldOne.image[j][i].getRed());
      image[j][i].setGreen(oldOne.image[j][i].getGreen());
      image[j][i].setBlue(oldOne.image[j][i].getBlue());
    }
  } 
}

Image& Image::operator=(const Image& oldOne){
  this->filename = oldOne.filename;
  allocateImage(oldOne.width, oldOne.height);
  for (unsigned int i = 0; i < oldOne.height; i++){
    for (unsigned int j = 0; j < oldOne.width; j++){
